...
import br.jabuti.agdtpoo.control.TestCaseMethodController;
import br.jabuti.agdtpoo.model.TestCaseCoverage;
import br.jabuti.agdtpoo.model.TestCaseMethod;
import br.jabuti.agdtpoo.model.TestCaseMethodCoverage;
...
public class TestCase {
	...
	private String prefix = new String("");
        ...
        private Hashtable<ProbedNode,String[][]> _trace = null;
	...
	public Coverage getTestCaseCoverageByPath(
        ...
        public void AGDTPOOupdateTestCaseCoverage(JabutiProject prj, TestCaseMethods methodsJUnit) {
		//if (testCaseCoverage[0].getNumberOfCovered() == 0) {
			...
			while (it.hasNext()) {
				...
				Iterator mthIt = methods.values().iterator();
                                ...
                                // Captura do método JUnit que será executado a partir do seu LABEL                                
                                TestCaseMethod currentMethod = TestCaseMethodController.getTestCaseMethodFromLabel(methodsJUnit, label);                            
                                ...
                                if (currentMethod != null)
                                {
                                    while (mthIt.hasNext()) {
                                            ...
                                            // Criação de nova instância de cobertura
                                            TestCaseMethodCoverage testCaseMCover = new TestCaseMethodCoverage();
                                            testCaseMCover.SetMethodName(cm.getMethodName());
                                            ...
                                            for (int i = 0; i < Criterion.NUM_CRITERIA; i++) {
                                                    ...
                                                    TestCaseCoverage testCaseCover = new TestCaseCoverage();
                                                    testCaseCover.setCriterion(criterion.getClass().toString());
                                                    testCaseCover.setRealizedCoverage(covered.size());

                                                    testCaseMCover.AddTestCaseMethodCoverage(testCaseCover);
                                            }

                                            currentMethod.AddTestCaseMethodCoverage(testCaseMCover);
                                    }
                                }
                                else
                                {
        ...
        public Hashtable<ProbedNode,String[][]> getTrace()
        {
            return this._trace;
        }
        
        public void setTrace(Hashtable<ProbedNode,String[][]> trace)
        {
            this._trace = trace;
        }
        
        public String getCoveredPath(String className, String methodName)
        {
            if (this._trace == null)
                return "";
            ...
            for (Enumeration e = this._trace.keys(); e.hasMoreElements();)
            {
                
                ProbedNode node = (ProbedNode)e.nextElement();
                
                String nodeX = node.clazz;
                
                if (nodeX.equals(className))
                { 
                    if (String.valueOf(node.metodo).equals(methodName) || methodName.equals(""))
                    {
                        String[][] path = this._trace.get(node);
                        for (String[] subPaths : path) {
                            for (String minimalPath : subPaths) {
                                coveragePath += minimalPath + "-";
                            }
                        }
                    }
                }
            ...
            //if (coveragePath.endsWith("-"))
            //coveragePath = coveragePath.substring(0, coveragePath.length() - 1);
