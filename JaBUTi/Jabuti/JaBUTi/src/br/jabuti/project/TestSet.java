...
import br.jabuti.agdtpoo.model.MethodStructure;
...
public class TestSet {
    ...
    public static TestCaseMethods executePartialTestCoverage(JabutiProject prj, String classUnderTest, String methodUnderTest, String methodUnderTestId) {
        
        return executeTestCoverage(prj, classUnderTest, methodUnderTest, methodUnderTestId, false) ;
        
    }
    
    public static TestCaseMethods executeTestCoverage(JabutiProject prj, String classUnderTest, String methodUnderTest, String methodUnderTestId) {
        
        return executeTestCoverage(prj, classUnderTest, methodUnderTest, methodUnderTestId, true) ;
        
    }
    
    private static TestCaseMethods executeTestCoverage(JabutiProject prj, String classUnderTest, String methodUnderTest, String methodUnderTestId, boolean excludeTraceFile) {
            TestCaseMethods methodsJUnit;                 
            TraceReader dtr = null;

            boolean useRequirementMap = !methodUnderTestId.equals("");

            setTraceFile(new File(prj.getTraceFileName()));
            ...
            if ((traceFile != null) && (traceFile.exists()) && (traceFile.isFile())) {
                    try {
                            ...
                            if (newTCs.size() > 0) {
                                    // Instância lista de métodos unitários
                                    methodsJUnit = new TestCaseMethods();

                                    for (int i = 0; i < newTCs.size(); i++) {

                                            // Cria nova instância para armazenar os testes unitários e suas respectivas coberturas.
                                            TestCaseMethod newTestCaseMethod = new TestCaseMethod();   

                                            // Atribui o ID do método
                                            newTestCaseMethod.SetId(i);

                                            TestCase tc = (TestCase) newTCs.elementAt(i);
                                            TestSet.activateTestCase(prj, tc.getLabel(), methodUnderTest, newTestCaseMethod, useRequirementMap);

                                            // Captura a cobertura do método sob teste
                                            String coveragePath = tc.getCoveredPath(classUnderTest, methodUnderTestId);

                                            newTestCaseMethod.SetName(tc.getAlias());     
                                            newTestCaseMethod.SetLabel(tc.getLabel());  

                                            // Atribui a cobertura registrada  cobertura do método sob teste
                                            newTestCaseMethod.setMethodUnderTestCoverage(coveragePath);

                                            methodsJUnit.add(newTestCaseMethod);
                                    }
    ...
    public static void activateTestCase(JabutiProject prj, String label) {
        activateTestCase(prj, label, "", null, false);
    }

    /**
     * Make an existent test case active
     */
    public static void activateTestCase(JabutiProject prj, String label, String methodUnderTest, TestCaseMethod testCaseMethod, boolean useRequirementMap) {
            if (!isActive(label)) {
                    ...
                    while (it.hasNext()) {
                            ...
                            while (mthIt.hasNext()) {
                                    ...
                                    boolean saveNewMethod = false;

                                    ClassMethod cm = (ClassMethod) mthIt.next();
                                    Criterion criterion = null;

                                    // Fernando Henrique - 16/09/2012
                                    // Verifica se o método já foi mapeado
                                    RequirementMap reqGuide = null;
                                    String methodName = UtilBehaviors.getMethodSignature(cm.getMethod().toString());

                                    // É preciso mapear os requisitos de cada método
                                    if (useRequirementMap)
                                    {
                                        reqGuide = RequirementMap.getInstance();

                                        if ((methodUnderTest.equals(methodName) || methodUnderTest.equals("")))
                                            saveNewMethod = !reqGuide.containsMethodStructure(methodName);
                                    }
                                    ...
                                    for (int i = 0; i < Criterion.NUM_CRITERIA; i++) {
                                        ...
                                        // Fernando Henrique - 16/09/2012
                                        // Identifica os requisitos dos diferentes critérios
                                        if (useRequirementMap)
                                        {   
                                            // Captura o nome do critério
                                            String nameCriterion = criterion.names[i][0];
                                            // Converte o nome do critério para uma enumeração
                                            EnumCriterion selectedCriterion = UtilBehaviors.getCriterion(nameCriterion);

                                            // Registra um método e seus respectivos requisitos
                                            if (saveNewMethod)
                                            {
                                                methodStructure.addCriterionAndRequirements(selectedCriterion, criterion.getRequirements());   
                                            }

                                            // Registra um test case 
                                            if (testCaseMethod != null && (methodUnderTest.equals(methodName) || methodUnderTest.equals("")))
                                            {

                                                // Captura todas as coberturas do requisito
                                                HashSet coverage = criterion.getPossibleCoveredRequirements(label);
                                                // Adiciona o critério e sua cobertura ao  aso de teste
                                                testCaseMethod.AddCriterionCoverage(selectedCriterion, coverage.toArray(new Object[coverage.size()]));
                                            }                                                    
                                        }                                                
                                    }

                                    // Fernando Henrique - 16/09/2012
                                    // Registra o método sob teste na classe geral de registros de tipos e critérios
                                    if (saveNewMethod)
                                    {
                                        reqGuide.addMethodStructure(methodStructure);
                                    }
                            }
                    }
            }
     ...
     * Creates a new TestCase instance and adds it to the current test set
     */
    public static TestCase createNewTestCase(JabutiProject prj, String label) {
            TestCase tc = new TestCase(prj, TestSet.newTestCaseLabel(), label);
            TestSet.addTestCase(prj, tc,
            // true,
                            false, false);
    ...
    public static void AGDTPOOupdateTestSetCoverage(JabutiProject prj, TestCaseMethods methodsJUnit) {
            for (int i = 0; i < Criterion.NUM_CRITERIA; i++) {
                    testSetCoverage[i] = prj.getProjectCoverage(i);
            }
            ...
            while (it.hasNext()) {
                    ...
                    tc.AGDTPOOupdateTestCaseCoverage(prj, methodsJUnit);
            }
    }

    public static void AGDTPOOupdateOverallCoverage(JabutiProject prj, TestCaseMethods methodsJUnit) {
            ...
            TestSet.AGDTPOOupdateTestSetCoverage(prj, methodsJUnit);
    }
