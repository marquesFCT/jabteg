...
public class DefaultProbeInsert {
    ...
    private ArrayList<InstrumentClass> _classes;    
        
    public InstrumentClass[] getInstrumentClasses()
    {
        return _classes.toArray(new InstrumentClass[_classes.size()]);
    }
    ...
    public DefaultProbeInsert(Program p, Collection c) {
        ...
        this._classes = new ArrayList<InstrumentClass>();
    }
        ...
        for (int i = 0; i < userClass.length; i++) {
            ...
            if ( (!achou) && inst.contains(userClass[i]) && ! jv.isInterface()) 
            {
                ...
                try {
                    ...
                    jv = doDefaultInstrument(jv, userClass[i], typeOfCFG, i);
	            hs.put(r.getName(), jv);
    ...
    private JavaClass doDefaultInstrument(JavaClass java_class, 
        ...
        InstrumentClass insClass = new InstrumentClass();
        
        insClass.setName(className);
        insClass.setId(classId);
...
//						+ methods[i].getSignature());          
                ...
                InstrumentMethod insMethod = new InstrumentMethod();                
                insMethod.setId(i);
                insMethod.setName(methods[i].toString());
...
//                gfc.releaseInstructionGraph(); // free some memory
                ...
                if (methods[i].getName().equals("<init>")) {
                    ...
                    GenerationRequirement newRequirement = new GenerationRequirement(String.valueOf(superNode.getNumber()));
                    for (Object object : superNode.getPrimNext()) {                                
                        GraphNode node = (GraphNode)object;
                                
                        newRequirement.addNextRequirement(String.valueOf(node.getNumber()));
                    }
                    
                    insMethod.addRequirement(newRequirement);
...
//                        System.out.println("Inicio do noh: " + ey.getStart());
                        if (!jahFoi.contains(ih)) {
                            ...
                            insMethod.addRequirement(new GenerationRequirement(String.valueOf(ey.getNumber())));
                        }                       
                    ...
                    for (int m = 0; m < gfc.size(); m++) {
                        ...
                        if (gfc.isEntry(gn) )
                        {
                        	String s = "invokestatic " + getProbeClass() 
                                + " getNest \"()J\"" 
                            + " lstore " + nextLocal;
                            //if (gi == null || ih == null || s == null)
                            //{
                               // String algumDosControlesEhNull = "true";
                            //}
                        ...
                        if (!jahFoi.contains(ih)) {
                            ...
                            // Fernando Henrique - 11/08/2012
                            GenerationRequirement newRequirement = new GenerationRequirement(String.valueOf(gn.getNumber()));
                            for (Object object : gn.getPrimNext()) {                                
                                GraphNode node = (GraphNode)object;
